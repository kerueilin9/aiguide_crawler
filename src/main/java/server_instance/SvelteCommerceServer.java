...
public class SvelteCommerceServer extends ServerInstanceManagement {
    ...
    public SvelteCommerceServer(String appName, int server_port) {
    ...
    private void createDockerComposeFile() {
        ...
        compose_file = dockerFolder + "docker_compose_svelte_commerce_" + (server_port % 3000) + ".yml";
        try {
    ...
    public void createServerInstance() {
        ...
        while(!isServerActive(String.format(url, server_port), 200)) {
            ...
            else if(waitingCount == MAXIMUM_WAITING_COUNT * 2) throw new RuntimeException("Something went wrong when creating svelte_commerce...");
        }
    }
