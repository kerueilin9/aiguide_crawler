...
public class WagtailsServer  extends ServerInstanceManagement {
    ...
    public WagtailsServer(String appName, int server_port) {
        super(appName, server_port);
        createDockerComposeFile();
        copyVE();
        CodeCoverageCollector codeCoverageCollector = new PythonServerCodeCoverageCollector(server_port);
    ...
    private void createDockerComposeFile() {
        ...
        String compose_file_content = "wagtails_with_coverage_%d:\n" +
                " image: ntuthongkaihuang/wagtail\n" +
                " ports:\n" +
                "  - \"127.0.0.1:%d:8000\"";
        compose_file_content = String.format(compose_file_content, server_port % 3000, server_port);
        compose_file = dockerFolder + "wagtails_with_coverage_" + (server_port % 3000) + ".yml";
        try {
